...
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Installed SVD surface gate passes against certified references. | Red. Tiny-Gram rows certify and pass memory but miss speed. Retained IRLBA/LBD now has a residual-augmented native diagnostic that certifies the H-shaped wide row directly and beats retained block-GK source-loaded, but it is still slower and higher-allocation than certified RSpectra and remains unpromoted. |
