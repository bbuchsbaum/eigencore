...
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Installed SVD surface gate passes against certified references. | Red. The executable `--h-candidate` gate now targets `eigencore_irlba_lbd_retained_bpro` instead of retained block-GK, and retained BPRO reuses accepted native certificate diagnostics at return. Installed 3-iteration tall/wide sparse probes certify all five requested triplets, but fail speed (`0.140x` tall, `0.177x` wide) and memory (`0.170x` tall, `0.162x` wide) versus the best certified references. Tiny-Gram and implicit-normal diagnostics remain bounded diagnostic paths, not H promotion. |
| I randomized SVD | `inst/benchmarks/bench-randomized-rsvd.R` | Randomized gate passes only against certified `rsvd` baseline rows. | Mixed, not promoted. Large exact-low-rank dense and quick low-rank sparse rows are green with native dense/CSC sketch/projection diagnostics active; quick exact-low-rank dense and slow-decay/native solver-control regimes remain open. |
