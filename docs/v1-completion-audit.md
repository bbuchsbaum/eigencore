...
| I randomized SVD | `inst/benchmarks/bench-randomized-rsvd.R` | Randomized gate passes only against certified `rsvd` baseline rows. | Mixed, not promoted. Large exact-low-rank dense row is green; direct dense/CSC projected-core diagnostics are active, but fresh quick exact-low-rank and sparse rows remain below the `2x` gate and slow-decay/native sketch regimes remain open. |
