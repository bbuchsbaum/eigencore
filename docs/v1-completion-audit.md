...
This is the final stop-rule checklist for the active V1 readiness goal. It
keeps completion tied to evidence instead of effort, intent, or a green proxy
command.
...
|---|---|---|---|
| Clean package checks | `eigencore_0.0.0.9000.tar.gz` and `eigencore.Rcheck/` | `LC_ALL=C LANG=C R CMD build /Users/bbuchsbaum/code/eigencore`; `LC_ALL=C LANG=C R CMD check --as-cran --no-manual eigencore_0.0.0.9000.tar.gz` | Green on final 2026-06-05 local run; `R CMD check` reports `Status: 1 NOTE` for CRAN incoming feasibility (`New submission`, development version). |
| Full test suite | `tests/testthat/` | `Rscript -e 'pkgload::load_all("."); testthat::test_dir("tests/testthat", reporter="summary")'` | Green on final 2026-06-05 local run with four expected CRAN skips. |
...
| Benchmark manifest | `docs/v1-benchmark-manifest.md` | Every release surface maps to installed commands, saved artifact names, and current gate status. | Green for promoted solver gates; final aggregate evidence is recorded in `benchmarks/RELEASES.md`. |
...
The future solver families remain listed as V1 boundaries; they are not hidden
release blockers for the scoped surface.
