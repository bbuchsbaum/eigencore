...
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Installed SVD surface gate passes against certified references. | Red. Tiny-Gram rows certify and pass memory but miss speed. Retained IRLBA/LBD now has residual-augmented, cached-`A Q`, BPRO native diagnostics, and exact-final lock diagnostics that certify the H-shaped wide row directly and beat retained block-GK source-loaded, but they are still slower and higher-allocation than certified RSpectra and remain unpromoted. |
