...
|---|---|---|---|
| Clean package checks | `eigencore_1.0.0.tar.gz` and fresh `eigencore.Rcheck/` output | `LC_ALL=C LANG=C R CMD build /Users/bbuchsbaum/code/eigencore`; `LC_ALL=C LANG=C R CMD check --as-cran --no-manual eigencore_1.0.0.tar.gz` | Green on 2026-07-15 from a fresh source tarball after the final pre-CRAN solver changes: `0 ERROR`, `0 WARNING`, `1 NOTE` for CRAN new submission. Tarball SHA-256: `cf89c9a273bfef0c1384fa23319391c05b665e498b64eca4203d1572b805951f`. |
| Full test suite | `tests/testthat/` | `Rscript -e 'pkgload::load_all("."); testthat::test_dir("tests/testthat", reporter="summary")'`; installed `tests/testthat.R` inside the source-tarball check | Green on 2026-07-15: the installed-package `testthat.R` pass in the fresh 1.0.0 CRAN-like check was OK. |
...
| G1 Hermitian native gate | `inst/benchmarks/bench-native-hermitian-gate.R` | Clean installed strict gate and saved artifacts. | Green on fresh installed 2026-08-24 evidence for the promoted factorized tridiagonal default. At `path_laplacian:1000`, `k = 20`, eigencore certified `20/20`, ran at `0.00621s`, was `7.54x` faster than RSpectra, and passed the retained-result envelope at `0.873x`; the `n = 10000` row also certified `20/20` and passed at `716x` speed and `0.986x` retained-result ratio. `bench::mem_alloc` remains a separately printed R-allocation diagnostic (`0.412x`), not a cross-engine peak-memory claim, because it excludes native heaps. |
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Installed SVD surface gate and target contracts pass against certified references. | Green for the promoted non-quick tall/wide sparse H surface plus sparse smallest/interior target contracts. Fresh installed 2026-07-15 strict evidence passes matched `tol` to RSpectra and irlba and certifies `20/20` triplets for every method on `tall_sparse` (`100000 x 500`) and `wide_sparse` (`500 x 100000`). Eigencore uses materialized `lapack_dsyevr` with no fallback and passes certified-total speed/memory versus RSpectra at `2.16x` / `2.05x` tall and `7.18x` / `2.15x` wide. A controlled fixed-small-side sweep locates certified-total crossover by long side 5,000 in both orientations while keeping raw and certified timing distinct. Fresh 2026-06-06 source strict target evidence still covers smallest/interior target contracts. The `complex_dense` benchmark row covers the native dense complex SVD label outside the sparse H gate. |
...
As of 2026-07-15, the fresh local source-tarball gate is green. The remaining
final-validation items are fresh GitHub Actions, R-hub, and win-builder
R-release/R-devel results for this exact release candidate; earlier remote
results cover an older package payload and cannot close the gate.
