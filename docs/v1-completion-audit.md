...
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Installed SVD surface gate passes against certified references. | Red. The executable `--h-candidate` gate now targets `eigencore_irlba_lbd_retained_bpro` instead of retained block-GK. Installed 3-iteration tall/wide sparse probes certify all five requested triplets, but fail speed (`0.124x` tall, `0.196x` wide) and memory (`0.168x` tall, `0.160x` wide) versus the best certified references. Tiny-Gram and implicit-normal diagnostics remain bounded diagnostic paths, not H promotion. |
