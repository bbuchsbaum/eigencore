...
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Installed SVD surface gate passes against certified references. | Red. Tiny-Gram rows certify and pass memory but miss speed. Retained IRLBA/LBD now has residual-augmented, cached-`A Q`, BPRO native diagnostics, exact-final lock diagnostics, and opt-in guarded BPRO reorth/block diagnostics that certify the H-shaped wide row directly and beat retained block-GK source-loaded, but they are still slower and higher-allocation than certified RSpectra and remain unpromoted. |
| I randomized SVD | `inst/benchmarks/bench-randomized-rsvd.R` | Randomized gate passes only against certified `rsvd` baseline rows. | Mixed, not promoted. Large exact-low-rank dense row is green; native CSC sketch/projection diagnostics are active and lower sparse allocation, but fresh quick rows remain below the `2x` gate and slow-decay/native solver-control regimes remain open. |
