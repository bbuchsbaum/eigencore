...
| Package checks clean | `eigencore_1.0.0.tar.gz` tarball result | `LC_ALL=C LANG=C R CMD build /Users/bbuchsbaum/code/eigencore`; `LC_ALL=C LANG=C R CMD check --as-cran --no-manual eigencore_1.0.0.tar.gz` | Green on 2026-07-15 from a fresh source tarball after the final pre-CRAN solver changes: `0 ERROR`, `0 WARNING`, `1 NOTE` for CRAN new submission. Tarball SHA-256: `cf89c9a273bfef0c1384fa23319391c05b665e498b64eca4203d1572b805951f`. |
| Full test suite clean | `tests/testthat/` | `Rscript -e 'pkgload::load_all("."); testthat::test_dir("tests/testthat", reporter="summary")'`; installed `tests/testthat.R` inside the source-tarball check | Green on 2026-07-15: the installed-package `testthat.R` pass in the fresh 1.0.0 CRAN-like check was OK. |
| Diff hygiene | whole repo | `git diff --check` | Green on 2026-07-15 after the pre-CRAN solver fixes and release-candidate metadata refresh. |
| CRAN remote checks | GitHub Actions, R-hub, win-builder | GitHub Actions status for the release candidate; R-hub Ubuntu release and Windows; win-builder R-release and R-devel email results | Fresh remote checks are required for the 2026-07-15 candidate. Earlier results do not sign off the new payload; GitHub Actions, R-hub, and win-builder R-release/R-devel must all be rerun against this release candidate before submission. |
...
| Hermitian G1 default | `R/problem.R`, `R/solve.R`, `R/transform_shift_invert.R`, `src/scalar_krylov.cpp`, `R/certification.R`, `inst/benchmarks/bench-native-hermitian-gate.R`, `benchmarks/RELEASES.md` | strict Hermitian benchmark script; planner/result/certificate tests | Green for the promoted factorized tridiagonal default. Fresh installed 2026-08-24 evidence certifies `20/20` at both `path_laplacian:1000` and `:10000`. The `n = 1000` row runs at `0.00621s`, is `7.54x` faster than RSpectra, and passes the bounded retained-result gate at `0.873x`; the `n = 10000` row runs at `0.0231s` and passes at `716x` speed and `0.986x` retained-result memory. Reused parsed bands and native selected-Ritz return reduced cumulative R allocation at `n = 1000` from 3.82 MB to 1.51 MB. That allocation ratio remains diagnostic rather than a peak-memory claim because R profiling excludes native heaps. Native block Hermitian Lanczos remains explicit/diagnostic rather than promoted for general sparse `auto()`. |
| H production SVD | `R/reference_golub_kahan.R`, `R/solve_svd.R`, `R/validation.R`, `inst/benchmarks/bench-svd-surface.R`, `inst/benchmarks/bench-svd-gram-cutoff.R`, `tests/testthat/test-svd-adversarial.R`, `benchmarks/RELEASES.md` | SVD surface gates and target contracts against certified references | Green for the promoted non-quick tall/wide sparse H surface plus sparse smallest/interior target contracts. The harness passes matched `tol` to RSpectra and irlba, reports raw and common certified-total timing separately, and includes a nested fixed-small-side crossover mode. Fresh installed 2026-07-15 evidence certifies `20/20` for every method on `100000 x 500` and its transpose; eigencore uses materialized `lapack_dsyevr` with no fallback and passes certified-total speed/memory versus RSpectra at `2.16x` / `2.05x` tall and `7.18x` / `2.15x` wide. The controlled sweep finds certified-total crossover by long side 5,000 in both orientations, while raw tall crossover occurs by 25,000. The right-normal implicit probe is opt-in and exact-certificate failure retries explicit Gram before Golub-Kahan. Fresh 2026-06-06 source target evidence still covers smallest/interior contracts. Retained IRLBA/LBD/BPRO diagnostics remain unpromoted beyond this H surface. |
