...
| H production SVD | `R/reference_golub_kahan.R`, `R/solve_svd.R`, `R/validation.R`, `inst/benchmarks/bench-svd-surface.R`, `tests/testthat/test-svd-adversarial.R`, `benchmarks/RELEASES.md` | SVD surface gates against certified references | Not complete. The SVD surface script supports stable `--cases=` ids and progress messages. Native CSC Gram special cases now have fast-result construction on both left- and right-Gram sides, so the tall CSC path avoids R result/certificate assembly. This is useful overhead reduction, not promotion: fresh installed 2026-05-17 3-iteration quick probes still miss the speed gate (`0.40x` on `tall_sparse:600x90`, `0.63x` on `wide_sparse:90x600`) while certifying and passing memory. The retained IRLBA/LBD diagnostic now has a residual-augmented native projection path with cached `A Q` reuse: source-loaded `wide_sparse:90x600` certifies directly with `restart_state_kind = residual_augmented_projection`, recurrence and augmented-recurrence flags set, `24` scout matvecs, `92` retained-native matvecs versus `136` for full post-hoc basis application, one residual direction, `40` tail steps, `46` cached augmented basis columns, and `8` prefix small SVDs. The BPRO variant also certifies directly with a monitored one-pass default, `4` threshold-triggered second passes, `53` total reorthogonalization passes versus `98` for full retained native, exact basis loss below `1e-15`, no escalation recommendation, clustered/slow-decay adversarial coverage, and exact-final lock diagnostics that report hard/soft counts, lock source, certified-triplet status, orthogonality status, and fallback reason while leaving `retained_deflation = FALSE`. Opt-in one-sided-guarded and block-guarded BPRO diagnostics now expose reorth mode, one-sided use, block size, exact orthogonality loss/pass, and fallback reason; both remain non-default and certificate-gated. A seeded larger `200x2000` sparse-wide fixture also certifies directly. The source-loaded H row is faster than retained block-GK, but it remains slower and higher-allocation than the best certified reference (`RSpectra`) and slower than direct one-sided GK; random larger sparse probes can still fall back. H remains unpromoted. |
| I randomized SVD | `R/reference_golub_kahan.R`, `inst/benchmarks/bench-randomized-rsvd.R`, `docs/hegelsvd_svd_acceleration.md`, `benchmarks/RELEASES.md` | randomized/rsvd accuracy and time-to-certified-answer gates | Not complete. The randomized-rsvd script supports stable `--cases=` ids and progress messages, and its gate now requires the `rsvd` baseline itself to satisfy eigencore certification before parity can pass. Fresh installed evidence separates green regimes from the broader open gate: non-quick `exact_low_rank_dense:2000x500` certifies against certified `rsvd` and passes speed/accuracy at about `3.15x`; native dense/CSC sketch/projection diagnostics are benchmark-visible as `randomized_native_sketch = TRUE` and `randomized_projection_kind = "native_direct_qt_a"` for eigencore rows; quick `low_rank_sparse:140x90` now certifies, allocates less than `rsvd`, and passes speed in the full quick surface at about `2.01x`. Quick exact-low-rank dense remains just below gate at about `1.93x`, and `slow_decay_dense:140x90` certifies eigencore while the `rsvd` baseline fails eigencore certification and therefore records `baseline_certified = FALSE`, `speed_gate = FALSE`, and `passed = FALSE`. Broader slow-decay/native solver-control gates remain open. |
