...
| Mote handoff | `.mote/` issue state | `mote board`, `mote ls`, issue histories | In progress. Completed solver slices are noted; release-hardening blockers remain open. |
...
- N: release docs now include RSpectra migration, method/workflow selection,
  ...
  README/vignette refresh after release-hardening signoff are incomplete.
  Solver gates have fresh installed evidence; final aggregate signoff still
  needs to bind those artifacts together.
