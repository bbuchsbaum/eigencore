...
ABI without weakening planner honesty. The implementation remains available
for explicit block requests and diagnostic sparse gate reruns, but sparse
`auto()` promotion is disabled until the non-quick G1 gate is green again.
...
- `plan_solver()` labels explicit block runs, dense full-subspace block runs,
  and diagnostic sparse block opt-in runs as
...
The promoted solver forms block-recurrence residuals, maintains a structured
...
benchmark accounting without double-certifying eigencore rows. Fresh installed
non-quick evidence on `path_laplacian:1000` is red on speed and PRIMME parity,
so sparse `auto()` promotion is currently demoted to diagnostic opt-in through
`options(eigencore.promote_sparse_block_lanczos = TRUE)`. A post-demotion
installed rerun confirms the default sparse planner now uses the scalar path
(`block_size = 1`), but that scalar default row is still red on speed, memory,
and PRIMME parity. The projected-matrix update now updates only newly appended
basis columns during expansion, reducing the installed scalar row from about
`5.41s` to about `0.327s`, but the row still misses release speed and parity
thresholds. Installed quick gates remain diagnostic for small `k`;
`--quick --strict` enforces certification only, while non-quick `--strict`
enforces the release speed/memory/parity gates. Dense quick rows are opt-in
diagnostics via `--include-dense`.
...
explicit block rows. Older `n = 1000, k = 20` and `n = 10000, k = 20` sparse
rows were positive under earlier benchmark conditions, but the fresh installed
rerun supersedes those as release evidence. The default `eigencore` sparse
planner now reports scalar Lanczos; block labels appear only for explicit block
requests, dense full-subspace cases, and diagnostic sparse opt-in runs.
