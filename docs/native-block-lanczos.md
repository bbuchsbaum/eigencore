...
ABI without weakening planner honesty. The implementation is now the promoted
G1 block solver in benchmark-proven regimes.
...
- `plan_solver()` labels promoted block runs as
...
- The benchmark script reports an explicit promotion gate. On the current quick
  ...
  `16*k` restart space for promoted large sparse `k >= 16` rows, with a
...
benchmark accounting without double-certifying eigencore rows. It is now the
promoted G1 solver for benchmark-proven regimes. Installed quick gates remain
diagnostic for small `k`; `--quick --strict` enforces certification only, while
non-quick `--strict` enforces the release speed/memory/parity gates. Dense quick
rows are opt-in diagnostics via `--include-dense`.
...
explicit block rows. The `n = 1000, k = 20` sparse row passes the RSpectra
...
uses the robust large-sparse reference controls. The default `eigencore` strict
gates now pass on the sparse and dense Hermitian regression suite, and the
planner reports the production block label for promoted runs.
