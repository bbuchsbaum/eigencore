...
`auto()` promotion for the block path remains disabled until its own non-quick
...
The block solver forms block-recurrence residuals, maintains a structured
...
benchmark accounting without double-certifying eigencore rows. It remains
explicit/diagnostic for sparse problems through
`options(eigencore.promote_sparse_block_lanczos = TRUE)`. The installed
2026-06-05 G1 gate for `path_laplacian:1000` is green through the promoted
structured tridiagonal default, which dispatches to the native selected
tridiagonal solver and tridiagonal residual certificate. That gate did not
promote the block path. Installed quick gates remain diagnostic for small `k`;
...
rerun supersedes those as block-path release evidence. The default `eigencore`
sparse planner now reports the selected tridiagonal solver for tridiagonal
sources and scalar Lanczos for other sparse Hermitian sources; block labels
appear only for explicit block requests, dense full-subspace cases, and
diagnostic sparse opt-in runs.
