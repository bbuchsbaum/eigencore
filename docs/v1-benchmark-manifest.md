...
| H production SVD | `inst/benchmarks/bench-svd-surface.R` | Executable retained H candidate: `R_LIBS_USER=/tmp/eigencore-bench-lib Rscript inst/benchmarks/bench-svd-surface.R --quick --iterations=3 --subject=eigencore_irlba_lbd_retained_bpro --methods=eigencore_irlba_lbd_retained_bpro,RSpectra,PRIMME,irlba,rsvd --cases=tall_sparse:600x90,wide_sparse:90x600`; default `--h-candidate` now selects the same retained BPRO subject. Diagnostic tiny-Gram checks may still use `--subject=eigencore --methods=eigencore,RSpectra,irlba,rsvd`; normal-scout diagnostics may use `--methods=eigencore_irlba_lbd_normal_scout,eigencore_golub_kahan_one_sided,RSpectra --subject=eigencore_irlba_lbd_normal_scout`. | `*-svd-surface-rows.rds`, `*-svd-surface-gates.rds`, `*-svd-surface-memory.rds` | Red. Fresh installed 2026-06-05 retained BPRO H probes certify all five requested triplets on `tall_sparse:600x90` and `wide_sparse:90x600`, but fail speed (`0.140x` tall, `0.177x` wide) and memory (`0.170x` tall, `0.162x` wide) versus the best certified references. Native tiny-Gram and implicit-normal rows remain useful diagnostics, and retained IRLBA/LBD still exposes residual augmentation, cached `A Q` certificate reuse, BPRO monitoring, guarded BPRO modes, exact-final lock diagnostics, and accepted native certificate diagnostic reuse, but none of those paths is promoted as the general H production answer. |
| I randomized SVD | `inst/benchmarks/bench-randomized-rsvd.R` | `R_LIBS=/tmp/eigencore-bench-lib Rscript inst/benchmarks/bench-randomized-rsvd.R --iterations=1 --save --cases=exact_low_rank_dense:2000x500`; quick diagnostics: `--quick --iterations=1 --save --cases=exact_low_rank_dense:120x80,slow_decay_dense:140x90,low_rank_sparse:140x90` | `*-randomized-rsvd-rows.rds`, `*-randomized-rsvd-gates.rds` | Mixed. Fresh installed non-quick exact-low-rank row is green at about `3.15x` versus certified `rsvd`; a fresh installed 5-iteration quick rerun after adding native dense/CSC randomized sketch/projection kernels reports `randomized_native_sketch = TRUE` and `randomized_projection_kind = "native_direct_qt_a"` for eigencore rows, with quick low-rank sparse certified, lower-allocation, and green at about `2.01x` in the full quick surface. Quick exact-low-rank dense remains just below gate at about `1.93x`, and slow-decay still cannot pass parity when the `rsvd` baseline fails eigencore certification (`baseline_certified = FALSE`, `passed = FALSE`). I is not fully promoted. |
