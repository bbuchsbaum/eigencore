...
The release audit and completion audit carry the matching tracked status map
for scoped current surfaces, future-scope boundaries, and V3 deferrals.
...
that every adjacent future solver is complete.

## Solver Limitations

| Area | Current limitation |
|---|---|
| Hermitian eigen | Standard symmetric tridiagonal sparse/diagonal Hermitian sources with algebraic edge targets (`largest()`/`smallest()`) now auto-route to native factorized shift-invert Lanczos with original-coordinate certificates. The native selected tridiagonal LAPACK path remains explicitly reachable/internal for reference and regression coverage, not the advertised `auto()` path. Explicitly tagged internal 2D path-grid Laplacian operators have a diagnostic separable prototype boundary for smallest eigenpairs; installed 2026-06-07 evidence on `100x100`, `k = 10` certifies in original coordinates and beats RSpectra, but this is not automatic recognition for arbitrary sparse Hermitian matrices. Native scalar Hermitian Lanczos remains the default for non-tridiagonal sparse paths. Native block Hermitian Lanczos exists for explicit requests and diagnostic sparse opt-in, but is not promoted for general sparse `auto()`. |
| SVD | Promoted non-quick tall/wide sparse largest-SVD rows pass the installed H gate through bounded native Gram special cases. The default Gram-SVD materialization gates are `eigencore.gram_svd_max_dimension = 512` for tall (right-Gram) inputs and `eigencore.gram_svd_max_dimension_wide = 1024` for wide (left-Gram) largest-target inputs; all runs must also pass `eigencore.gram_svd_memory_mb`, rank-fraction, aspect-ratio, work-budget, and original-coordinate certificate gates. Installed 2026-06-11 cutoff evidence (`inst/benchmarks/results/20260611-svd-gram-cutoff-*`) certifies 600/768/1024 small-side tall and wide sparse rows; wide rows beat the best baseline 1.8–2.6× and are promoted to the 1024 wide default, while tall rows remain slower than RSpectra on the gate cases, so the tall default stays at 512 (larger runs opt in via the option). Sparse CSC smallest-SVD targets use a native certified Gram special case when the smaller Gram dimension is inside the explicit materialization gate, and otherwise retain the native certified Golub-Kahan smallest boundary; exact-norm matrix-free callbacks keep their native smallest callback label. Nearest/interior SVD has a native full-subspace Golub-Kahan boundary for sparse CSC inputs and exact-norm matrix-free callbacks, while dense smallest and nearest-sigma requests use exact dense fallback certificates and diagonal/reference prototype interior rows can certify without densification. Scalable refined/shift-invert interior SVD remains future scope. Real matrix-free operators with adjoints have a native callback-cycle Golub-Kahan label and sidecar gate, but quick `600x90` probes are diagnostic only, and general sparse/matrix-free production thick-restart SVD remains future scope. |
...
| Generalized eigen (non-SPD pencil) | Dense full generalized pencils (`eig_full()`, `generalized_schur()`, `generalized_svd()`) use native dense LAPACK with original-coordinate certificates, left eigenvectors, conditioning diagnostics where LAPACK exposes them, and `pencil_norm_scaled` alpha/beta classification for badly scaled finite/infinite/undefined pairs. The real dense GSVD surface calls LAPACK `dggsvd`, so it requires a linked LAPACK that still provides that deprecated routine. Sparse partial general pencils with nonsingular diagonal `B` use native transformed Arnoldi on `B^{-1} A` without densification for the gated real-case target families; sparse general-pencil `smallest()` on graph-Laplacian-like near-null spectra is not a promoted Fiedler path. Singular, general sparse `B`, and sparse QZ remain explicit unsupported boundaries with fail-loud messages rather than silent dense fallback. General sparse CSC SPD partial paths (diagonal/CSC/tridiagonal `B`, constraints) live under the generalized LOBPCG/Lanczos surfaces in `bench-generalized-lobpcg.R`. |
...
| Shift-invert | Dense standard, dense generalized SPD, diagonal standard, sparse symmetric-tridiagonal standard, and sparse/diagonal tridiagonal generalized paths are native and pass the scoped V1 installed gate. `auto()` with `target = nearest(sigma)` routes through an implicit `shift_invert(sigma)` transform for supported factorized Hermitian regimes and fails loudly for matrix-free/unfactorized inputs without a solve. Standard symmetric tridiagonal `largest()`/`smallest()` auto routes also use implicit native shift-invert, choosing zero for Laplacian-style lower edges and a stable Gershgorin-edge shift when the edge is away from zero. All shift-invert paths carry `shift_invert_factorization_contract_v1`: native labels require eigencore-owned factorized apply, general sparse uses an honest `Matrix::lu` reference contract, and user solves use an external-cache contract. Native general sparse LU and native ownership of user solve functions are explicit release non-goals unless a future scope revision reopens them. |
| Hermitian warm starts | `initial_subspace` is supported only for standard real Hermitian Lanczos: native explicit dense-double and `dgCMatrix` paths, the native matrix-free callback path selected by `lanczos(block > 1)`, and the scalar reference-labelled matrix-free path selected by `lanczos(block = 1)`. Generalized, shift-invert, dense-fallback, nonsymmetric, and other non-Lanczos plans reject it instead of ignoring it; use explicit `lanczos()` when a start is required because `auto()` may choose shift-invert. Starts are validated, orthonormalized, rank-fitted, and recorded in result diagnostics, but remain hints: every solve recomputes projected quantities and a fresh certificate. `check_stride = 0L` preserves full-sweep behavior; a positive stride enables mid-sweep convergence checks only on native block paths and does not add operator applications. A residual certificate does not prove target identity, so a fully supplied subspace that is already invariant at the requested tolerance is discarded by the invariant guard; continuation starts should overlap the requested target eigenspace. |
...
| Matrix-free operators | Supported through callback boundaries. Explicit real Hermitian `lanczos(block > 1)` requests use the native block thick-restart callback kernel, while scalar `block = 1` retains the reference Hermitian path. The real nonsymmetric Arnoldi callback path and real matrix-free SVD callback path also have native callback labels and sidecar gates; callback-driven paths remain separate from built-in native kernels unless a gate explicitly promotes them. |
...
general-pencil boundary, not graph/Fiedler guidance. Dense full generalized pencils,
`generalized_schur()`, and `generalized_svd()` carry native dense LAPACK labels.
Sparse general pencils with nonsingular diagonal `B` carry
`native transformed sparse general-pencil Arnoldi`; this label is a
general-pencil boundary, not graph/Fiedler guidance, and all other sparse
general pencils carry explicit unsupported labels.
