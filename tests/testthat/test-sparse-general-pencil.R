...
test_that("sparse diagonal-B general pencils use transformed Arnoldi", {
  ...
  expect_equal(coords$beta, rep(1, 2L))
  expect_identical(coords$classification_policy$policy, "exact_beta_one")
  expect_identical(coords$classification_policy$tolerance, 0)
  expect_match(coords$classification_policy$reason, "finite by construction")
