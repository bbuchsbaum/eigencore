...
1.6× RSpectra, 9.5× irlba, 2.7× PRIMME (see below).
...
- **First-class operator algebra over a native engine.** Dense, CSC,
  ...
  loop.
- **Kernel-level numerics discipline.** Certificate Gram products use
  `dsyrk` (symmetric rank-*k* update) rather than `dgemm`, halving the
  flop count. Arnoldi refined-Ritz extraction hoists all workspace out
  of the per-eigenpair loop and forms Ritz vectors with two `dgemv`
  calls — a measured 3× speedup at n = 10⁵. Full reorthogonalization in
  the scalar Lanczos cycle uses block CGS2 (`dgemv`) matching the
  existing Golub-Kahan path, with adversarial-bank evidence of
  equivalent orthogonality quality.
