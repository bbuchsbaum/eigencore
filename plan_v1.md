...
1. **H production SVD.** The scoped V1 H gate is closed for the promoted
   ...
   residual-augmented projection consumes it. The residual-augmented native
   path now carries the accepted exact certificate diagnostics forward to the
   returned result instead of rerunning the same cached-`A Q` certificate during
   final packaging; focused tests compare the reused certificate to direct
   original-operator recomputation. The remaining native gap is now
