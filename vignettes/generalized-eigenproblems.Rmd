...
stopifnot(
  ...
  all(coordinates$classification == "finite")
)
```

For a general pencil, right vectors satisfy `A v = lambda B v`, while left
vectors satisfy the adjoint equation. Inspect both certificates when the left
basis matters to sensitivity or biorthogonal projections:
...
```{r validate-left-vectors, include=FALSE}
left_residual <- Conj(t(W)) %*% A_general -
  diag(values(pencil)) %*% (Conj(t(W)) %*% B_general)
stopifnot(
  pencil$left_certificate$passed,
  max(Mod(left_residual)) < 1e-10
)
```

The classification policy is part of the result because “finite” depends on
the numerical contract used to interpret `(alpha, beta)`. Do not replace it
with an unrecorded comparison such as `abs(beta) < 1e-8`.
...
possibly rectangular matrices act on the same column direction, and you want
to compare the strength of those two actions. The matrices must have the same
number of columns, but they may have different numbers of rows.

For diagonal maps, the generalized singular values are easy to anticipate:
each value is the corresponding strength in `A` divided by the strength in
`B`.
...
```{r validate-gsvd-diagonal, include=FALSE}
stopifnot(
  certificate(gsvd_fit)$passed,
  isTRUE(all.equal(
    sort(values(gsvd_fit)), sort(c(3 / 4, 4 / 3, 5 / 2)),
    tolerance = 1e-10
  )),
  isTRUE(all.equal(
    gsvd_coordinates$alpha^2 + gsvd_coordinates$beta^2,
    rep(1, 3),
    tolerance = 1e-12
  ))
)
...
Here `alpha^2 + beta^2 = 1` for every pair. `alpha / beta` is finite whenever
`beta` is positive, even when it is very small. The returned
`classification_policy` records that structural rule separately from `tol`,
which controls reconstruction and orthogonality certification.

Rank-deficient rectangular pairs can contain finite, infinite, and undefined
structural positions at once:
...
```{r validate-gsvd-rank-layout, include=FALSE}
stopifnot(
  certificate(gsvd_rect)$passed,
  length(values(gsvd_rect)) == ncol(A_rect),
  any(rect_coordinates$infinite),
  any(rect_coordinates$undefined),
  sum(is.na(values(gsvd_rect))) == ncol(A_rect) - gsvd_rect$rank
)
```

`Inf` means `beta = 0` while `alpha` is nonzero. `NA` represents a trailing
structural `(0, 0)` pair, not an accidentally missing computation. Keeping the
full length-`ncol(A)` layout lets classifications, factor columns, and
reconstruction metadata stay aligned. Use `left_vectors(gsvd_fit)` and
`right_vectors(gsvd_fit)` for the two row-space factors; the shared column
factor remains `gsvd_fit$Q`.
...
`generalized_schur()` for QZ, `generalized_svd()` for a GSVD, and
`alpha_beta()` to inspect homogeneous coordinates.
...
- `?generalized_svd` documents the returned factors and reconstruction layout.
